...
export function AppShell({copy, locale}: {copy: Copy; locale: Locale}) {
  ...
  return <div className="shell">
    <aside aria-label={copy.home.navigation} className="sidebar">
      ...
      <nav aria-label="Primary" className="nav">{items.map((item) => <a aria-current={item === "home" ? "page" : undefined} className="nav__item" href={`/${locale}`} key={item}><span aria-hidden="true">{icons[item]}</span><span>{copy.nav[item]}</span></a>)}</nav>
      <div className="sidebar__footer"><ThemeSwitcher /><LocaleSwitcher locale={locale} /></div>
    </aside>
    <main className="workspace">
      <header className="topbar"><div><p className="eyebrow">{copy.home.eyebrow}</p><h1>{copy.home.hello}</h1></div><button className="avatar" type="button" aria-label="Profile">AS</button></header>
    ...
    <nav className="mobile-nav" aria-label="Mobile navigation">{items.slice(0, 4).map((item) => <a aria-current={item === "home" ? "page" : undefined} href={`/${locale}`} key={item}><span aria-hidden="true">{icons[item]}</span><small>{copy.nav[item]}</small></a>)}</nav>
  </div>;
}
